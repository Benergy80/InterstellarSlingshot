...
export function createDemo({ player, planet, transit, npcs, fx, hud, camera }) {
  ...
  const log = [];
  const stats = { loops: 0, tasksDone: 0, teleports: 0, deaths: 0 };
  let active = false;
  ...
  // ── spherical navigation with wall avoidance ──
  // Straight-line bearings die in the warrens; probe ahead and swing
  // the bearing to the first clear direction (Venice steering).
  function clearBearing(bearing) {
    _v2.copy(st.pos).addScaledVector(_up, 1.0);
    if (!planet.probe(_v2, bearing, 2.6)) return bearing;
    for (const ang of [0.65, -0.65, 1.3, -1.3, 1.9, -1.9]) {
      _v3.copy(bearing).applyAxisAngle(_up, ang);
      if (!planet.probe(_v2, _v3, 2.6)) return _v3;
    }
    return bearing;   // boxed in — let the stuck ladder handle it
  }
  ...
  // walk toward a point; returns remaining distance (3D)
  function walkTo(p, dt, { run = true } = {}) {
    steerToward(p, dt);
    key('w', true);
    if (run) key('b', distTo(p) > 18 && st.energy > 30);
    else key('b', false);
    return distTo(p);
  }
  ...
  return {
    ...
    report() {
      ...
      return { active, stats: { ...stats }, byType, issues: [...log] };
    },
    update(dt, t, fps) {
      if (!active) return;
      fpsEMA = fps ?? fpsEMA;
