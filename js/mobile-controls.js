...
// Ensure mobile controls load - debug logger
console.log('📱 mobile-controls.js: Starting to load...');

// Pre-define critical functions to prevent "not defined" errors
window.mobileEmergencyWarp = window.mobileEmergencyWarp || function() { console.error('mobileEmergencyWarp not ready'); };
window.startForwardThrust = window.startForwardThrust || function() { console.error('startForwardThrust not ready'); };
window.stopForwardThrust = window.stopForwardThrust || function() { console.error('stopForwardThrust not ready'); };
window.mobileBrakesStart = window.mobileBrakesStart || function() { console.error('mobileBrakesStart not ready'); };
window.mobileBrakesEnd = window.mobileBrakesEnd || function() { console.error('mobileBrakesEnd not ready'); };
window.showMobilePanel = window.showMobilePanel || function() { console.error('showMobilePanel not ready'); };
window.hideMobilePanel = window.hideMobilePanel || function() { console.error('hideMobilePanel not ready'); };

// Use IIFE to prevent any variable conflicts
(function() {
    'use strict';
    ...
    // Touch controls - isolated in IIFE scope
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    // Swipe gesture - isolated in IIFE scope
    let swipeStartX = 0;
    ...
    console.log('📱 Mobile controls system loaded successfully (IIFE protected)');
})();

// Verify all mobile functions are loaded
(function verifyMobileControls() {
    ...
    if (missing.length > 0) {
        console.error('❌ Missing mobile functions:', missing);
    } else {
        console.log('✅ All mobile control functions loaded successfully');
    }
})();
